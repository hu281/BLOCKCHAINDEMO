...
class PeerToPeer {
    ...
    closeConnection() {
        this.peers.forEach(peer => {
            try {
                ...
                console.log(`Error closing peer connection: ${err.messages}`);
            }
        });
        this.peers = [];
    ...
    broadcastLatest() {
        this.broadcast(Messages.sendLatestBlock(this.blockchain.sendLatestBlock));
    }
    broadcast(message) {
        this.peers.forEach(peer => {
    ...
    initConnerction(connection) {
        ...
        if (!existingPeer) {
            ...
            this.write(connection, Messages.getLatesBlock());
